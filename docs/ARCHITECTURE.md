...
- `repo_privacy_guardian/gui/state.py`: pure Audit/Repair flow state helpers for GUI button labels, gate notes, repair summaries, option-row specs, path-field specs, action-button specs, collapsible-section visibility, and responsive Reports/Prompts/Identity layout decisions
