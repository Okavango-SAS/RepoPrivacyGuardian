...
`repo_privacy_guardian/core.py` is no longer the 12k-line monolith, but it remains the compatibility nexus for the `1.x` public API. The first bridge cleanup moved `redaction`, `tooling`, and `reporting` from broad core star imports to explicit dependencies. Reporting also imports cleanly as a standalone module instead of relying on a partially initialized core cycle. The remaining debt is to keep replacing transitional bridges in scanner and GUI modules with narrower explicit dependencies once each slice has domain-level tests. Future extraction should continue by small slices with regression tests after each slice and no behavior drift in CLI/GUI parity.
