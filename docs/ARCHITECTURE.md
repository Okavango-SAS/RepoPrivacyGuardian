...
- `repo_privacy_guardian/gui/state.py`: pure Audit/Repair flow state helpers for GUI button labels, gate notes, repair summaries, option-row specs, setup option-menu specs, path-field specs, entry-field specs, action-button specs, heading/label/status-panel specs, dynamic empty-state/prompt-card/report-artifact presentation, collapsible-section visibility, and responsive Reports/Prompts/Identity layout decisions
