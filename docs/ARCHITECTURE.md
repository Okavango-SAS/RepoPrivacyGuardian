...
`repo_privacy_guardian/core.py` is no longer the 12k-line monolith, but it remains the compatibility nexus for the `1.x` public API. The bridge cleanup moved `redaction`, `tooling`, `reporting`, and `scanner` from broad core star imports to explicit dependencies. Reporting and scanner also import cleanly as standalone modules instead of relying on partially initialized core cycles. The remaining debt is to keep replacing the transitional GUI bridge with narrower explicit dependencies once that slice has domain-level tests. Future extraction should continue by small slices with regression tests after each slice and no behavior drift in CLI/GUI parity.
