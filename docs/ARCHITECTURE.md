...
- `repo_privacy_guardian/gui/assets.py`: GUI raster asset and icon adapter for loading, dark-mode theming, cached image lookup, registered label refresh, and window icon updates
- `repo_privacy_guardian/gui/background.py`: GUI background-worker adapter for daemon worker startup, UI-thread scheduling, and blocking yes/no prompts that must be marshalled back to the Tk event loop
- `repo_privacy_guardian/gui/dialogs.py`: GUI file/directory dialog adapter for initial-directory resolution and browse/save target updates
- `repo_privacy_guardian/gui/locale.py`: GUI text catalogs, tooltips, and font-selection helpers
- `repo_privacy_guardian/gui/navigation.py`: GUI flow-tab navigation adapter for direct tab selection, locale renames, visual order preservation, and active-tab remapping
- `repo_privacy_guardian/gui/state.py`: pure Audit/Repair flow state helpers for GUI button labels, gate notes, repair summaries, option-row specs, setup option-menu specs, Audit/Repair card specs, path-field specs, entry-field specs, action-button specs, heading/label/status-panel specs, dynamic empty-state/prompt-card/report-artifact presentation, collapsible-section visibility, and responsive Reports/Prompts/Identity layout decisions
- `repo_privacy_guardian/gui/theme.py`: pure GUI theme helpers for palette snapshots, theme-token translation, widget option updates, and special-widget update maps
- `repo_privacy_guardian/gui/window.py`: GUI window lifecycle helpers for root creation, CTk theme initialization, geometry sizing, appearance-mode callbacks, and mainloop dispatch
...
`repo_privacy_guardian/core.py` is no longer the 12k-line monolith, but it remains the compatibility nexus for the `1.x` public API. The bridge cleanup moved `config`, `redaction`, `tooling`, `reporting`, `policy`, `remediation`, `scanner`, and `gui/app` from broad core star imports or core-owned helpers to explicit dependencies, and `gui/locale` now owns its locale identifiers and shared GUI text constants. Config, evidence taxonomy, execution, history parsing, reporting, policy, remediation, scanner, GUI app, GUI assets, GUI background, GUI dialogs, GUI locale, GUI navigation, GUI state, GUI theme, and GUI window helpers also import cleanly as standalone modules instead of relying on partially initialized core cycles. Remediation now owns the pure `git-filter-repo` rewrite command plan, execution owns the side-effecting subprocess/Git adapters used by scanner, history parsing owns pure `git log -p` line parsing and finding formatting, evidence taxonomy owns the bucket aggregation for high-confidence, low-confidence, fixture, and documentation secret findings, GUI assets owns raster/icon loading and theme refresh behavior, GUI background owns daemon worker startup plus UI-thread callback marshalling, GUI dialogs owns browse-dialog path targeting, GUI navigation owns direct flow-tab selection and locale rename remapping, GUI state owns pure Audit/Repair button, gate-state, setup option-menu specs, card specs across Audit/Settings/Reports/Prompts/Repair/repository-list surfaces, collapsible-section visibility, and Reports/Prompts responsive layout decisions, GUI theme owns pure palette translation plus special-widget update maps, and GUI window owns root creation, geometry, appearance callbacks, and mainloop dispatch. The remaining debt is to keep moving behavior-bearing GUI coordinator and runtime adapter logic out of the compatibility nexus once each slice has domain-level tests. Future extraction should continue by small slices with regression tests after each slice and no behavior drift in CLI/GUI parity.
