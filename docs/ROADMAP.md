...
- continue extracting `repo_privacy_guardian/core.py` by the remaining CLI parser surface and deeper scanner execution seams without breaking compatibility shims; config, redaction, tooling, reporting, policy, remediation, scanner, GUI app, and GUI locale already use explicit dependencies or local constants
