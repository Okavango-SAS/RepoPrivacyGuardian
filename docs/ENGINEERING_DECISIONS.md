...
- Continue replacing transitional compatibility bridges in extracted modules with narrower explicit dependency injection; config, redaction, tooling, execution, history parsing, reporting, policy, remediation, scanner, GUI app, and GUI locale now use explicit dependencies or local constants. Remediation owns pure `git-filter-repo` command planning, execution owns the side-effecting Git/subprocess adapters, including streaming history process lifecycle, and history parsing owns pure `git log -p` line parsing/formatting; the next candidates are scanner history taxonomy aggregation and GUI support helpers that still live in the compatibility nexus.
