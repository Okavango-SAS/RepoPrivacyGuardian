...
- Continue replacing transitional compatibility bridges in extracted modules with narrower explicit dependency injection; redaction, tooling, reporting, scanner, and GUI app now use explicit dependencies. The next candidates are GUI locale constants and policy/remediation helpers that still live in the compatibility nexus.
