...
- Continue replacing transitional compatibility bridges in extracted modules with narrower explicit dependency injection; config, redaction, tooling, reporting, policy, remediation, scanner, GUI app, and GUI locale now use explicit dependencies or local constants. The next candidates are the full CLI parser surface and deeper scanner execution seams that still live in the compatibility nexus.
