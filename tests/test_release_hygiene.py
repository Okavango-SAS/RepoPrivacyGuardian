...
def test_gui_assets_module_imports_without_core_dependency() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "-c",
            (
                "import sys; "
                "import repo_privacy_guardian.gui.assets as gui_assets; "
                "print(gui_assets.ICON_SIZE); "
...
def test_gui_theme_module_imports_without_core_dependency() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "-c",
            (
                "import sys; "
                "import repo_privacy_guardian.gui.theme as gui_theme; "
                "print(gui_theme.parse_hex_rgb('#FFFFFF')); "
                "print('repo_privacy_guardian.core' in sys.modules)"
            ),
        ],
        cwd=str(_repo_root()),
    ...
    assert proc.returncode == 0, proc.stderr or proc.stdout
    assert proc.stdout.splitlines() == ["(255, 255, 255)", "False"]
...
def test_changelog_records_stable_release() -> None:
    ...
    assert "Post-release GUI, locale, and cleanup hardening update." in changelog
