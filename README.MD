...
It is designed for repeatable CLI execution first, with an optional desktop GUI that runs on the same underlying pipeline. Developed and maintained by **Okavango SAS**.

## 🌎 Language / Idioma

**Español (Latinoamérica).** Repo Privacy Guardian te ayuda a revisar repositorios Git antes de publicarlos: detecta secretos, metadatos de identidad Git, rutas personales, archivos sensibles e inconsistencias de `.gitignore`, y genera evidencia local en JSON, HTML y log. La CLI se mantiene en inglés para preservar compatibilidad con automatizaciones, pero la GUI permite elegir `Español (Latinoamérica)` sin cambiar el comportamiento ni los reportes.
...
**English.** Repo Privacy Guardian audits Git repositories before public release: it detects secrets, Git identity metadata, personal paths, sensitive files, and `.gitignore` drift, then writes local JSON, HTML, and log evidence. The CLI remains English for automation compatibility, while the GUI can switch between `English` and `Español (Latinoamérica)` without changing behavior or reports.

## 🖥️ Desktop GUI Preview

![Repo Privacy Guardian desktop GUI audit screen](docs/ux-audit/after/audit-default-desktop-after.png)

The desktop GUI is optional and shares the same audit engine, policy keys, report schema, and safety gates as the CLI. Maintained, sanitized GUI screenshots live under `docs/ux-audit/`.
...
Read Audit_Results/<run_id>/report.json, report.html, and run.log.
...
All run evidence stays local under `Audit_Results/<run_id>/` as `report.json`, `report.html`, and `run.log`.
...
- First-time operator: delegate the 60-second agent prompt above, then inspect `Audit_Results/<run_id>/report.html`.
...
- `v1.4.1` hardened roadmap and CI trigger coverage for release-readiness docs.
- `v1.4.2` hardened release harness byte-compile coverage.
- `v1.4.3` hardened GUI parity and agentic publication readiness.
- `v1.4.4` hardened public prompt-library hygiene.
- `v1.4.5` is the current patch release with root layout allowlist hardening.
...
- local maintainer release gate: `python scripts/release_readiness.py` before public release or tag creation, including dependency vulnerability audit of dev/GUI/remediation requirement files
...
1. install the base CLI in the agent environment
2. give the agent the target repository, root folder, and the 60-second prompt above
3. have the agent verify `--help` and run `repo-privacy-guardian --check-tooling`
4. have the agent run a dry-run audit against one repository
5. inspect the generated JSON, HTML, and log artifacts
6. approve a dry-run fix preview only after findings are classified
7. approve real fixes, history rewrites, or pushes only after reviewing the preview
...
GUI localization is presentation-only. The selectable locales are English and Spanish (Latin America), persisted as a non-secret GUI preference. Adding another GUI language should add catalog entries with the same keys; it must not rename CLI flags, report fields, policy keys, or `GuardRunConfig` mappings.
...
Repo Privacy Guardian is designed to be comfortable in multiple modes, but the primary automation use case is an agentic IDE or coding agent running the CLI. This includes Codex, Claude Code, Antigravity, GitHub Copilot, Cursor, and similar tools.

The recommended agentic shape is:

- the human chooses the target repository and risk tolerance
- the agent runs Repo Privacy Guardian from CLI
- the first pass is audit-only
- findings are classified before any repair
- destructive remediation requires explicit approval
- artifacts remain local under `Audit_Results/<run_id>/`

A safe default pattern is:

1. `repo-privacy-guardian --help`
2. `repo-privacy-guardian --check-tooling`
3. dry-run audit
4. reviewed dry-run fix plan
5. real fix only for the reviewed repos
6. re-run until `PASS`
...
- `repo_privacy_guardian_prompts.py`: GUI/README agentic prompt registry without desktop GUI imports
...
- `Repo_Privacy_Guardian.py`
- `pyproject.toml`
- `repo_privacy_guardian_*.py` support modules published as `py-modules`

Generated outputs, local agent meta material, release bundles, coverage files, build outputs, and audit artifacts belong in ignored local paths such as `.local-meta/`, `Audit_Results/`, `dist/`, and `*.egg-info/`, not in the public tracked root.
